...
var _ = Describe("Test config-policy-controller deployment", func() {
	AfterEach(func() {
		...
		Kubectl(c, "apply", "-f", case2ClusterManagementAddOnCRDefault)

		By("Cleaning up CSRs")
		Kubectl(c, "delete", "csr", "-l", "open-cluster-management.io/addon-name="+case2ManagedClusterAddOnName)
	})
	...
	It("should create the config-policy-controller deployment in hosted mode in user's custom namespace",
		func(ctx SpecContext) {
			...
			Kubectl(c, "apply", "-f", case2CMAAddonWithInstallNs)
			...
			for i, cluster := range managedClusterList[1:] {
				...
				Kubectl(c, "delete", "-n", cluster.clusterName, "-f", case2ManagedClusterAddOnCR, "--timeout=180s")
				deploy := GetWithTimeout(
	...
	It("should create a config-policy-controller deployment with node selector on the managed cluster",
		func(ctx SpecContext) {
			...
			By("Applying the config-policy-controller ClusterManagementAddOn to use the AddOnDeploymentConfig")
			Kubectl(c, "apply", "-f", case2ClusterManagementAddOnCR)
	...
	It("should create a config-policy-controller deployment with customizations from annotations",
		func(ctx SpecContext) {
			for _, cluster := range managedClusterList {
				...
				Kubectl(c, "annotate", "-n", cluster.clusterName, "-f", case2ManagedClusterAddOnCR, clientQPSAnnotation)
	...
	It("should create a config-policy-controller deployment with customizations from AddOnDeploymentConfig",
		func(ctx SpecContext) {
			...
			By("Applying the config-policy-controller ClusterManagementAddOn to use the AddOnDeploymentConfig")
			Kubectl(c, "apply", "-f", case2CMAAddonWithCustomizedVars)
